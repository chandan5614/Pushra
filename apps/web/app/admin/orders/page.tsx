...
export default function AdminOrders() {
  ...
  function onDragStart(id: string) { return () => setDragId(id) }
  function onDragOver(e: React.DragEvent) { e.preventDefault() }
  async function onDrop(target: UiStatus) {
    if (!dragId) return
    const newStatus = mapToServer(target)
    const res = await fetch(`/api/admin/orders/${dragId}`, { method: 'PATCH', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ status: newStatus }) })
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-4 gap-4">
        {uiCols.map((c) => (
          <div key={c} className="bg-white border rounded p-2 min-h-[300px]"
            ...
            <div className="space-y-2">
              {grouped[c].map((o) => (
                <div key={o.id} className="border rounded p-2 bg-gray-50 cursor-move"
                     draggable onDragStart={onDragStart(o.id)}>
