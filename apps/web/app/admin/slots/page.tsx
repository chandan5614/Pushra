...
type SlotRow = { id: string; label: string; capacity: number; reserved: number; cutoffAt?: string | null }
...
export default function AdminSlots() {
  ...
  const rows: SlotRow[] = useMemo(() => {
    ...
    return (day?.slots || []).map((s: any) => ({ id: s.id, label: s.label, capacity: s.capacity, reserved: s.reserved, cutoffAt: s.cutoffAt }))
  }, [calendar, date])

  async function saveInline(row: SlotRow) {
    const payload = { date, city, window: row.label, capacity: row.capacity, cutoffAt: row.cutoffAt || null }
    const res = await fetch('/api/admin/slots', { method: 'PUT', headers: { 'content-type': 'application/json' }, body: JSON.stringify(payload) })
    const j = await res.json(); setMsg(j?.ok ? 'Saved' : 'Failed')
    await load()
  }
                ...
                <tr key={r.id} className="text-sm">
                  ...
                  <td className="p-2 border-b">
                    <input
                      ...
                      onBlur={(e) => { r.capacity = parseInt(e.currentTarget.value, 10) || 0; saveInline(r) }}
                    />
                  </td>
                  ...
                  <td className="p-2 border-b">
                    <input
                      ...
                      onBlur={(e) => { r.cutoffAt = e.currentTarget.value || null; saveInline(r) }}
                    />
                  </td>
                  <td className="p-2 border-b">
                    <button className="px-3 py-1 rounded bg-gray-100" onClick={() => saveInline(r)}>Save</button>
                  </td>
                </tr>
              )
            })}
