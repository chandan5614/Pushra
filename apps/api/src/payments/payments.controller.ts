...
export class PaymentsController {
  ...
  @Post('checkout/init')
  async initCheckout(@Req() req: any, @Body() body: any) {
    ...
    if (provider === 'stripe') {
      ...
      if (provider === 'paytabs') {
        ...
        const session = await this.paytabs.createPayment(amountCents, currency, {
          orderId,
          returnUrl,
          callbackUrl,
        })
        ...
        const res = await this.testPay.createPayment({
          amountCents,
          currency,
          paymentId: paymentId!,
        })
